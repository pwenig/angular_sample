class PackageInputsController < ApplicationController
  ...
  def create
    @package_input = PackageInput.find_by(package_input_tag: params['package_input_tag'])
    if @package_input
  ...
  def show
    @package_input = PackageInput.find_by(package_input_tag: params[:id])
    if @package_input
      render json: @package_input,
             except: %i[agency_id publisher_id buy_method_id inventory_type_id],
             include: %i[agency publisher buy_method inventory_type],
             status: 200
    else
      head :no_content
    end
