...
RSpec.feature 'Creative Input', type: :feature, js: true do
  ...
  it 'duplicates a creative input' do
    pending
    date = Time.now.strftime('%Y').to_s + Time.now.strftime('%m').to_s + Time.now.strftime('%d').to_s + '-' + Time.now.strftime('%Y').to_s + Time.now.strftime('%m').to_s + Time.now.strftime('%d').to_s
    fill_in('email-login', with: @user.email)
    fill_in('password-login', with: @user.password)
    ...
    select('01', from: 'Creative Version Number')
    select('Copy', from: 'A/B Test Label')
    click_on('Save Creative String')
    expect(page).to have_text('CCL_CLTF_S00_AON_CS_President_01_CP_100x300_' + Time.now.strftime('%m').to_s + Time.now.strftime('%d').to_s + '-' + Time.now.strftime('%m').to_s + Time.now.strftime('%d').to_s)
    click_on('duplicateCreative')
    select('For Your Consideration', from: 'Creative Message')
    click_on('Save Creative String')
    expect(page).to have_text('CCL_CLTF_S00_AON_FC_President_01_CP_100x300_' + Time.now.strftime('%m').to_s + Time.now.strftime('%d').to_s + '-' + Time.now.strftime('%m').to_s + Time.now.strftime('%d').to_s)
  end
end
